...
hdul = fits.open('../../data/Buzzard_data/oliver_delta.fits')
...
hp.mollview(density_field_buzzard, min=-1, max=4)
hp.graticule()
plt.savefig(filepath_map+'C_plot_output/density_field_buzzard_map.pdf')

## HEALPY
for i in range(patch_count):
    pixel_id = random.randint(1, density_field_buzzard.size - 1)
    ...
    density_field_buzzard_patch = density_field_buzzard[pixels_indices_patch]
    ...
    del_data = Column(density_field_buzzard_patch, name='del', dtype='float') # column having pixels_patch_values

    density_fluctuations_table = Table((RA_data, DEC_data, del_data))  
    density_fluctuations_table.write(filepath_map+'/A_healpy_patches_produced/del_buzzard_patch_'+str(i+1)+'.fits', overwrite=True)
