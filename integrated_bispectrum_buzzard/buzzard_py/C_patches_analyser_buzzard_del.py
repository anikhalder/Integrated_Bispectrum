...
theta_mean_buzzard_map_vec = np.zeros(theta_vec.size)
w_mean_buzzard_map_vec = np.zeros(theta_vec.size)
mean_del_mean_buzzard_map_vec = 0
i_zeta_mean_buzzard_map_vec = np.zeros(theta_vec.size)

for i in range(patch_count):
    # read buzzard map's treecorr data
    theta_vec = np.loadtxt(filepath_map+'B_treecorr_patches_correlated/correlation_buzzard_patch_'+str(i+1)+'.txt', usecols=(0)) # in arcmins
    ...
    theta_mean_buzzard_map_vec = theta_mean_buzzard_map_vec + theta_vec
    w_mean_buzzard_map_vec = w_mean_buzzard_map_vec + w_vec
    mean_del_mean_buzzard_map_vec = mean_del_mean_buzzard_map_vec + mean_del
    i_zeta_mean_buzzard_map_vec = i_zeta_mean_buzzard_map_vec + i_zeta_vec
...
#plt.scatter(theta_mean_buzzard_map_vec[1:], i_zeta_mean_buzzard_map_vec[1:])

# -----------------------------------------------------------------------------------
# Compute variance of i_zeta over all patches in the buzzard map
theta_vec = np.loadtxt(filepath+'buzzard_map/B_treecorr_patches_correlated/correlation_buzzard_patch_'+str(1)+'.txt', usecols=(0)) # in arcmins

i_zeta_variance_buzzard_map_vec = np.zeros(theta_vec.size)
...
for i in range(patch_count):
    ...
    i_zeta_variance_buzzard_map_vec = i_zeta_variance_buzzard_map_vec + (i_zeta_mean_buzzard_map_vec - i_zeta_vec)**2
