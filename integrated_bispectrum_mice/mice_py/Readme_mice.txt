...
1. The projected matter density map from Mice (only one map!) is already present in data folder 

2. Depending on the size patch you need in each map - execute the following 3 python files:

A_healpy_patches_producer_mice_del.py <sq_degrees_patch> 
B_treecorr_patches_correlator_mice_del.py <sq_degrees_patch>
C_patches_analyser_mice_del.py <sq_degrees_patch>
