...
hdul = fits.open('../../data/Mice_data/mice_number_field.fits')
...
hp.graticule()
plt.savefig(filepath_map+'C_plot_output/density_field_mice_map.pdf')

## HEALPY
for i in range(patch_count):
    pixel_id = random.randint(1, density_field_mice.size - 1)
    ...
    density_field_mice_patch = density_field_mice[pixels_indices_patch]
    ...
    del_data = Column(density_field_mice_patch, name='del', dtype='float') # column having pixels_patch_values

    density_fluctuations_table = Table((RA_data, DEC_data, del_data))  
    density_fluctuations_table.write(filepath_map+'/A_healpy_patches_produced/del_mice_patch_'+str(i+1)+'.fits', overwrite=True)
